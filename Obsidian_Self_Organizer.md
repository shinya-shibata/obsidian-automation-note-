...
async function callGemini(prompt, model = "gemini-2.5-flash") {
    ...
    try {
        ...
        new Notice(`❌ Gemini API呼び出し失敗: ${e.message}`);
        throw e;
    }
}
...
async function generateTheories() {
    ...
    try {
        ...
        await app.vault.create(theoryPath, theory);
        new Notice(`🎉 新理論が創出されました！: ${theoryPath}`);
    } catch (e) {
...
async function runArxiv() {
    ...
    try {
        ...
        new Notice("🎉 arXiv調査結果を 99_System/arxiv_summary.md に保存しました。");
    } catch (e) {
...
※この理論が直面するであろう論理的破綻の可能性や、実証における限界。`;
    ...
    try {
        const result = await callGemini(prompt, "gemini-3.5-flash"); // 思考用最新モデル
        const timestamp = moment().format("YYYYMMDD_HHmmss");
        const integratedPath = `${BRAINSTORMING_DIR}/統合理論_${timestamp}.md`;

        await app.vault.create(integratedPath, result);
        new Notice(`🎉 新たな学術フレームワークが創出されました！: ${integratedPath}`);
    } catch (e) {
